# 光纤维护服务 Agent 系统 — 工业级完整设计方案 v7.1-Final
...
| 版本           | 日期           | 修订内容                                                     | 状态       |
| -------------- | -------------- | ------------------------------------------------------------ | ---------- |
| v1.0           | 2026-07-25     | 初版：完整架构设计                                           | ✅          |
| v2.0           | 2026-07-25     | 全面重构：10 项架构优化                                      | ✅          |
| v3.0           | 2026-07-25     | 13 项深度优化                                                | ✅          |
| v4.0           | 2026-07-25     | 10 项深度优化                                                | ✅          |
| v5.0           | 2026-07-25     | 10 项深度优化                                                | ✅          |
| v6.0           | 2026-07-25     | 10 项深度优化                                                | ✅          |
| v7.0-Final     | 2026-07-29     | 终版：10 项最终优化                                          | ✅          |
| **v7.1-Final** | **2026-07-30** | **三项关键升级：① RAG Embedding 升级为 bge-large-zh-v1.5（1024 维，中文 SOTA）；② LLM 三级梯度（14b 推理 / 7b 表述 / 3b 兜底）；③ 融入 NarratorValidator 程序化校验回环 + 增强型 Loop 审计追踪** | **✅ 终版** |
...
第五部分：参数防线设计
第六部分：主图编排与 Controlled Loop（含 Narrator 校验回环）
第七部分：Tool 层设计
第八部分：RAG 知识引擎（bge-large-zh-v1.5）
第九部分：实时事件与主动诊断
第十部分：降级与容错策略（三级 LLM 梯度）
第十一部分：安全设计
第十二部分：可观测性与审计
第十三部分：部署方案
第十四部分：开发路线图与复杂度预算
...
| LLM 能力有限 | 使用 qwen2.5 系列（本地 Ollama），非 GPT-4 级别 | 不能让 LLM 做复杂推理/计算；**三级梯度（14b/7b/3b）平衡能力与资源** |
...
- LLM 出错 → 翻译错误 → **被 NarratorValidator 校验层拦截** → 模板兜底（可控）
...
| P6     | 梯度降级       | 三级 LLM 梯度（14b→7b→3b）+ 规则兜底 + 离线缓存 | L0-L4 降级链                 |
| P7     | 事件驱动       | 既响应主动查询，也响应实时事件触发自动诊断      | WebSocket + Proactive        |
| P8     | 全链路可审计   | 每个请求从输入到输出完整记录，满足等保三级      | audit_trail + LangFuse       |
| **P9** | **输出可校验** | **LLM 输出必须经程序化校验，不合格则降级模板**  | **NarratorValidator [v7.1]** |
...
│  │(23 Tools │ │(bge-large│ │(PG Check │ │(3-Layer  │ │(SQLite +     │ │
│  │REST-only)│ │-zh+BM25) │ │pointer)  │ │+2 Assert)│ │TTL 5min)     │ │
│  └──────────┘ └──────────┘ └──────────┘ └──────────┘ └──────────────┘ │
└─────────────────────────────────────────────────────────────────────────┘
         │
         ▼
...
│  │(3-tier │ │(Checkpoint│ │(Event  │ │(Trace +│ │+ Grafana │ │REST    ││
│  │LLM+Emb)│ │+ Audit)  │ │Queue)  │ │Audit)  │ │(Metrics) │ │:8080   ││
│  └────────┘ └──────────┘ └────────┘ └────────┘ └──────────┘ └────────┘│
└─────────────────────────────────────────────────────────────────────────┘
```
...
| 组件                 | 选型                             | 版本              | 理由                                                        |
| -------------------- | -------------------------------- | ----------------- | ----------------------------------------------------------- |
| **LLM（Primary）**   | **qwen2.5:14b**                  | Ollama 0.6+       | **意图识别 + 分析推理 + 报告生成，中文推理能力最强**        |
| **LLM（Secondary）** | **qwen2.5:7b**                   | Ollama 0.6+       | **Narrator 表述 + 知识问答 + Query 改写，表述任务 7b 足够** |
| **LLM（Tertiary）**  | **qwen2.5:3b**                   | Ollama 0.6+       | **L2 最终降级兜底，极低资源占用（~2GB VRAM）**              |
| **Embedding**        | **bge-large-zh-v1.5**            | HuggingFace / TEI | **中文语义检索 SOTA，1024 维，MTEB 中文榜 Top**             |
...
│  │ • 报告反思    │   │              │   │              │        │
│  │              │   │              │   │              │        │
│  │ VRAM: ~10GB  │   │ VRAM: ~5GB   │   │ VRAM: ~2GB   │        │
│  │ 延迟: 2-4s   │   │ 延迟: 1-2s   │   │ 延迟: 0.5-1s │        │
│  └──────────────┘   └──────────────┘   └──────────────┘        │
│                                                                 │
│  降级路径: 14b 不可用 → 7b 接管推理 → 3b 兜底 → 纯模板           │
└─────────────────────────────────────────────────────────────────┘
```
...
| 模型        | 职责                           | 为什么不用其他                                               |
| ----------- | ------------------------------ | ------------------------------------------------------------ |
| qwen2.5:14b | 推理（意图/参数/分析/报告）    | 7b 在复杂多参数提取场景准确率不足（实测 84% vs 95%）；32b 资源占用过大 |
| qwen2.5:7b  | 表述（Narrator/追问/知识问答） | 表述任务不需要强推理，7b 语言流畅度足够；14b 做表述是资源浪费 |
| qwen2.5:3b  | 降级兜底                       | 保证 LLM 路径不完全中断；2GB VRAM 任何机器都能跑             |

### 3.4 Embedding 模型选型 [v7.1 变更]

| 维度            | bge-large-zh-v1.5（本方案） | nomic-embed-text（原 v7.0） |
| --------------- | --------------------------- | --------------------------- |
| 维度            | **1024**                    | 768                         |
| 中文 MTEB 排名  | **Top 3**                   | 中等                        |
| 中文语义理解    | **原生中文训练**            | 英文为主，中文迁移          |
| 最大 Token      | 512                         | 8192                        |
| 模型大小        | ~1.3GB                      | ~274MB                      |
| 推理速度（CPU） | ~60ms/条                    | ~30ms/条                    |
| 部署方式        | HuggingFace / TEI / Ollama  | Ollama 内置                 |

**选型理由：**

> 光纤运维知识库以**中文技术文档**为主（国标、厂商手册、操作规程），bge-large-zh-v1.5 在中文语义理解上显著优于 nomic-embed-text。1024 维向量在相似度检索精度上优于 768 维。虽然模型更大、推理更慢，但 RAG 检索不在请求热路径上（可异步预加载），60ms 的延迟完全可接受。**bge-large-zh 的中文能力使得 Vector 检索可以主导（权重 0.6），不再需要像 nomic 时代那样依赖 BM25 补偿。**
...
│   │   ├── narrator_validator.py  # 表述校验（程序化）[v7.1 新增]
│   │   ├── report_generator.py    # 报告生成（14b）
│   │   ├── report_evaluator.py    # 报告评估（Reflection，14b）
...
│   ├── rag/
│   │   ├── engine.py              # RAG 引擎（bge-large-zh + BM25）
...
│   ├── test_param_gate.py
│   ├── test_loop.py
│   ├── test_narrator_validator.py # [v7.1 新增]
│   └── test_degradation.py
├── docker-compose.yml
├── pyproject.toml
└── README.md
...
graph.add_conditional_edges(
    ...
    route_after_rule_engine,
    {
        ...
        "rule_miss": "intent_classifier",      # 未命中 → LLM 意图识别（14b）
    }
)
...
class MainGraphState(TypedDict):
    ...
    # Narrator 校验 [v7.1 新增]
    narration: Optional[str]                    # Narrator 原始输出
    narrator_validation_passed: Optional[bool]  # 校验是否通过
...
### 6.2 主图构建（含 Narrator 校验回环）[v7.1 升级]
...
def build_main_graph():
    ...
    graph.add_node("intent_classifier", intent_classifier_node)       # 14b
    ...
    graph.add_node("analysis_expert", analysis_expert_node)           # 14b
    graph.add_node("narrator", narrator_node)                         # 7b
    graph.add_node("narrator_validator", narrator_validator_node)     # [v7.1] 程序化
    graph.add_node("template_fallback", template_fallback_node)       # [v7.1] 模板兜底
    graph.add_node("report_generator", report_generator_node)         # 14b
    graph.add_node("report_evaluator", report_evaluator_node)         # 14b
    graph.add_node("batch_dispatcher", batch_dispatcher_node)
    graph.add_node("knowledge_qa", knowledge_qa_subgraph)             # 7b
...
### 6.4 判断/表述分离 + Narrator 校验回环 [v7.1 核心升级]
...
使用 qwen2.5:7b（Secondary），表述任务不需要 14b 的推理能力。
...
async def narrator_node(state: MainGraphState) -> dict:
    ...
    return {
        ...
        "narration": response.content,  # [v7.1] 保存原始输出供校验
        "llm_call_count": state.get("llm_call_count", 0) + 1,
    }
# src/nodes/narrator_validator.py  [v7.1 新增]
"""
Narrator 输出校验节点：程序化校验，零 LLM。
确保 LLM 表述未篡改规则判断的关键数据。

设计动机：
  即使 Prompt 中写了"不得修改数值"，LLM 仍有 ~1% 的概率
  将 3.2dB 写成"约3dB"或遗漏关键指标。在电信等保三级场景下，
  这 1% 不可接受。NarratorValidator 用程序化方式闭合这个缺口。
...
校验失败 → 路由到 template_fallback（模板兜底），而非重试 LLM。
"""
import re
...
async def template_fallback_node(state: MainGraphState) -> dict:
    """[v7.1] Narrator 校验失败时的模板兜底（零 LLM）"""
    from langchain_core.messages import AIMessage
...
### 6.5 analysis_expert（Loop 推理环节，使用 qwen2.5:14b）
...
# Primary LLM: qwen2.5:14b 用于分析推理（需要强推理能力）
...
async def analysis_expert_node(state: MainGraphState) -> dict:
    """分析专家 — ReAct Loop 推理环节。使用 14b 模型。"""
    loop_count = state.get("loop_count", 0)
    judgment = state.get("rule_judgment")
...
# ===== Embedding: bge-large-zh-v1.5 (1024维, 中文 SOTA) [v7.1 变更] =====
...
vector_retriever = vectorstore.as_retriever(
    search_kwargs={"k": 5, "score_threshold": 0.55}  # bge 分数分布，阈值上调
)

# BM25 检索（中文关键词，补充语义检索）
bm25_retriever = BM25Retriever.from_documents(all_documents, k=5)

# ===== 混合检索：Vector 权重更高（bge-large-zh 中文能力强）[v7.1 变更] =====
hybrid_retriever = EnsembleRetriever(
    retrievers=[vector_retriever, bm25_retriever],
    weights=[0.6, 0.4],  # bge-large-zh 中文能力强，Vector 主导
)
...
### 8.3 检索权重调整说明 [v7.1 变更]

| 维度            | v7.0（nomic-embed）  | v7.1（bge-large-zh-v1.5） | 变更理由                                      |
| --------------- | -------------------- | ------------------------- | --------------------------------------------- |
| Vector 权重     | 0.4                  | **0.6**                   | bge-large-zh 中文语义理解 SOTA，Vector 可主导 |
| BM25 权重       | 0.6                  | **0.4**                   | 不再需要 BM25 补偿中文弱点                    |
| score_threshold | 0.45                 | **0.55**                  | bge 归一化后分数分布更集中，阈值上调          |
| 英文标注        | 必须（补偿中文弱点） | **可选**（锦上添花）      | bge 原生中文训练，英文标注收益降低            |
...
def preprocess_for_ingest(text: str) -> str:
    """为中文文档追加英文关键词标注（bge-large-zh 下为可选增强）"""
...
LLM_CONFIG = {
    "primary": {
        "model": "qwen2.5:14b",
        "temperature": 0.1,
        "use_cases": ["intent_classification", "analysis_expert",
                      "report_generation", "report_evaluation"],
        "timeout": 15,
    },
    "secondary": {
        "model": "qwen2.5:7b",
...
| 输出层 | 敏感信息脱敏 + **Narrator 校验** | IP/端口按角色脱敏 + **NarratorValidator [v7.1]** |
...
# Token（按模型分）[v7.1]
token_usage = Counter("agent_token_usage_total", "Token消耗", ["node", "model"])

# 参数
param_failures = Counter("agent_param_failures_total", "参数解析失败", ["type"])

# Narrator 校验 [v7.1 新增]
narrator_validation_failures = Counter(
    "agent_narrator_validation_failures_total", "Narrator校验失败次数"
)
```
...
| 模块                                          | 代码量        | 占比     | 说明                               |
| --------------------------------------------- | ------------- | -------- | ---------------------------------- |
| 规则引擎 (rule_engine.py)                     | ~800 行       | 11%      | 25-30 条规则 + 模板                |
| Tool 层 (23 个 Tool)                          | ~1,200 行     | 17%      | 含 HTTP Client                     |
| 参数关卡 (param_gate.py)                      | ~400 行       | 6%       | 正则 + 校验                        |
| 主图 + 路由                                   | ~650 行       | 9%       | StateGraph + 条件边 + **校验回环** |
| 子图 (data_collector 等)                      | ~800 行       | 11%      | 4 个子图                           |
| 节点 (analysis/narrator/**validator**/report) | ~800 行       | 11%      | LLM 节点 + **NarratorValidator**   |
| RAG 引擎                                      | ~500 行       | 7%       | **bge-large-zh** + BM25 + 入库     |
| 事件系统                                      | ~400 行       | 6%       | WebSocket + Redis                  |
| 降级 + 缓存 + 熔断                            | ~550 行       | 8%       | **三级 LLM** + 容错层              |
| 安全 + 审计 + 可观测                          | ~450 行       | 6%       | 横切关注点                         |
| 配置 + 入口 + 工具                            | ~300 行       | 4%       | 胶水代码                           |
| **合计**                                      | **~6,850 行** | **100%** | —                                  |

"约束 LLM"的代码（规则引擎+参数关卡+降级+安全+**NarratorValidator**）占 **34%**，"业务功能"占 47%，"基础设施"占 19%。
...
| W2   | LLM 路径 + Loop                   | IntentClassifier(**14b**) + DataCollector + RuleJudgment + AnalysisExpert(**14b**) + Narrator(**7b**) + 四重终止 | "FIB-0012为什么变红" 触发 2 轮 Loop                          |
| W3   | **Narrator 校验** + 批量 + RAG    | **NarratorValidator + TemplateFallback** + Send 并发 + **bge-large-zh** RAG + PG Checkpointer | **校验失败 → 模板兜底**；批量 200 条 < 15s；RAG Recall@3 > 85% |
| W4   | 事件 + 主动诊断 + 缓存            | WebSocket Listener + Redis + Proactive SubGraph + SQLite Cache + L4 降级 | CRITICAL 告警 → 5s 内自动诊断                                |
| W5   | 安全 + 审计 + 报告 + **三级降级** | Prompt 注入防护 + 审计日志 + Reflection Loop + 报告模板 + **14b→7b→3b 降级链** | 安全测试通过；**停 14b → 30s 切 7b**                         |
...
| **bge-large-zh 部署资源不足** | 低   | 中   | 支持 CPU 推理（~60ms/query）；可独立 TEI 服务 |
| 后端接口变更                  | 低   | 高   | Tool 层抽象隔离；接口版本化                   |
| Loop 死循环                   | 低   | 中   | 四重终止保障 + 无进展检测                     |
| 并发性能不足                  | 中   | 中   | 连接池 + Send 并发 + 批量 API                 |
| **Narrator 校验误报**         | 低   | 低   | 校验规则保守设计；误报走模板（不影响正确性）  |
...
| **Narrator 校验**   | **数值篡改 100% 拦截**  | **构造 LLM 输出篡改场景** |
...
| 审计日志完整性               | 所有请求可追溯         |
| 敏感信息脱敏                 | 输出无明文 IP/密码     |
| **Narrator 数值校验 [v7.1]** | **数值篡改 100% 拦截** |

------

## 附录 A：与历史版本对比

| 维度          | v5.0         | v6.0         | v7.0-Final                | **v7.1-Final（本文）**                   |
| ------------- | ------------ | ------------ | ------------------------- | ---------------------------------------- |
...
| 向量库        | —            | ChromaDB     | ChromaDB                  | ChromaDB                                 |
| 响应性能      | 无约束       | SLA 分级     | SLA + L0 规则             | SLA + L0 规则                            |
| Loop 终止     | 三重         | 三重         | 四重                      | 四重                                     |
| 参数校验      | 四层         | 三层         | 三层+两断言               | 三层+两断言                              |
| **降级**      | 无           | 四级         | 五级                      | **五级（三级 LLM 梯度）**                |
| 安全          | 基础         | 基础         | 等保三级                  | 等保三级 + **Narrator 校验**             |
| 审计          | LangFuse     | 等保三级     | PG + LangFuse             | PG + LangFuse                            |
| 事件系统      | 无           | WebSocket    | WebSocket + Redis         | WebSocket + Redis                        |
| **RAG**       | bge          | nomic+BM25   | nomic+BM25(0.6)           | **bge-large-zh + BM25(0.4)**             |
| 代码量        | 未估算       | ~5800 行     | ~6600 行                  | **~6850 行**                             |
| 开发周期      | 4 周         | 5 周         | 6 周                      | **6 周**                                 |
...
| ADR-002     | **bge-large-zh-v1.5 [v7.1 变更]**          | **中文语义检索 SOTA，1024 维，Recall@3 > 90%**   | nomic-embed-text         | **中文检索质量不足，需 BM25 补偿**     |
| ADR-003     | **Vector 权重 0.6 > BM25 0.4 [v7.1 变更]** | **bge-large-zh 中文能力强，Vector 可主导**       | BM25 0.6                 | **bge 已解决中文弱点，无需 BM25 主导** |
...
| **ADR-011** | **三级 LLM 梯度 [v7.1]**                   | **14b 推理 + 7b 表述 + 3b 兜底，资源与质量平衡** | **单 7b**                | **推理能力不足 / 降级无梯度**          |
| **ADR-012** | **Narrator 校验回环 [v7.1]**               | **程序化校验 LLM 输出，消除最后 1% 幻觉**        | **信任 LLM Prompt 约束** | **工业级不可接受**                     |
...
| **NarratorValidator [v7.1]** | **Narrator 输出的程序化校验节点（零 LLM）**            |
...
| **三级 LLM 梯度 [v7.1]**     | **14b（推理）→ 7b（表述）→ 3b（兜底）的分层模型架构**  |
...
**v7.1 的核心升级理念：** v7.0 解决了"能不能用"的问题，v7.1 通过三级 LLM 梯度解决"降级有没有层次"的问题，通过 NarratorValidator 解决"LLM 翻译也出错怎么办"的最后一公里问题——**不增加架构复杂度，只增加可靠性深度。**
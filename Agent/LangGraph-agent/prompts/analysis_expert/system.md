...
本文件完全静态，不含模板变量（循环进度由 user 消息末尾状态栏提供，
保持 system 前缀稳定，KV Cache 友好）；字面 JSON 花括号在加载时由程序
统一转义，文件内保持可读的单花括号。
...
3. 补充数据的机会有限（见 user 消息末尾的“当前执行状态”），越接近预算上限越应直接输出结论
4. 禁止请求“已请求数据的工具”清单中已失败的相同查询
5. 每次补充必须有明确理由
6. 绝不编造数据

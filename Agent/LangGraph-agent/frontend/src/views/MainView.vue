  ...
  <div class="app-container">
    ...
    <header>
      <div class="brand">
        ...
        <div>
          <h1>光纤维护服务系统</h1>
          <small>FIBER OPS · LANGGRAPH AGENT · DIRKWU</small>
        </div>
      </div>
      <div class="top-status">
        <span class="pill"><i class="dot dot-g"></i>统计推送 WS :8766</span>
        <span class="pill"><i class="dot dot-g"></i>LangGraph Agent :8000</span>
    ...
    <footer>
      <span><b>需求规格说明书 v12.0</b>（LangGraph Agent 版）</span>
      <span>光纤维护系统</span>
...
import { getAllColoredFibers, getFiberScene, sendChatMessage, getStatus } from '../api/index.js'
...
const STORAGE_KEY = 'fm_lg_chat_state'

function loadChatState() {
  try { const raw = sessionStorage.getItem(STORAGE_KEY); if (raw) return JSON.parse(raw) } catch (e) { /* ignore */ }
  return null
}
...
function saveChatState() {
  try {
    ...
    if (currentSession.value) sessionMessages[currentSession.value] = messages.value
    sessionStorage.setItem(STORAGE_KEY, JSON.stringify({
      currentSession: currentSession.value, sessions: sessions.value, sessionMessages,
    }))
  } catch (e) { /* ignore */ }
}
  ...
  try {
    ...
    const fibers = (data.fibers || []).map(f => {
      ...
      return {
        ...
        span: '—', alarm: color === 'r' ? '紧急' : color === 'y' ? '次要' : '',
        color, slClass: '', alarmClass,
        src_ne_id: f.fiber.src_ne_id, dst_ne_id: f.fiber.dst_ne_id,
      }
    })
    allFibers.value = fibers
    const colorLookup = {}
    ;(data.fibers || []).forEach(f => { colorLookup[f.fiber.fiber_id] = colorMap[f.color] || 'g' })
    ...
    renderTopology()
  } catch (e) { console.error('Failed to fetch colored fibers:', e) }
}
...
function updateNEPairs(scenes, colorLookup) {
  ...
  scenes.forEach(s => {
    ...
    if (!fiber) return
    const a = 'NE-' + fiber.src_ne_id, b = 'NE-' + fiber.dst_ne_id
...
function renderTopology() {
  ...
  scenes.forEach(s => {
    ...
    if (interFiber) {
      neSet.add(interFiber.src_ne_id); neSet.add(interFiber.dst_ne_id)
      allFibers.push({ fiberId: interFiber.fiber_id, src_ne_id: interFiber.src_ne_id, dst_ne_id: interFiber.dst_ne_id, src_board_id: interFiber.src_board_id, dst_board_id: interFiber.dst_board_id, isInter: true, color: colorLookup[interFiber.fiber_id] || 'g' })
    }
    if (isFull) {
      ;(s.scene.ne_internal_fibers || []).forEach(f => {
        allFibers.push({ fiberId: f.fiber_id, src_ne_id: f.src_ne_id, dst_ne_id: f.dst_ne_id, src_board_id: f.src_board_id, dst_board_id: f.dst_board_id, isInter: false, color: 'g' })
      })
    }
  })
  const neList = Array.from(neSet).sort((a, b) => a - b)
  const nePositions = {}
  const cols = 2, neBoxW = 190, neBoxH = 116, padX = 24, padY = 28, rowGap = 130
  neList.forEach((neId, idx) => {
    const col = idx % cols
    nePositions[neId] = { x: col === 0 ? padX : 660 - padX - neBoxW, y: padY + Math.floor(idx / cols) * rowGap }
    neActiveBoards[neId] = []; nePassiveBoards[neId] = []
  })
  scenes.forEach(s => {
    if (!s || !s.found || !s.scene) return
    if (s.scene.src_active_board && neActiveBoards[s.scene.src_active_board.ne_id]) neActiveBoards[s.scene.src_active_board.ne_id].push(s.scene.src_active_board.board_id)
    if (s.scene.dst_active_board && neActiveBoards[s.scene.dst_active_board.ne_id]) neActiveBoards[s.scene.dst_active_board.ne_id].push(s.scene.dst_active_board.board_id)
    ;(s.scene.passive_boards || []).forEach(b => { if (nePassiveBoards[b.ne_id]) nePassiveBoards[b.ne_id].push(b.board_id) })
  })
  const topoFibers = allFibers.map(f => {
    ...
    const midX = (srcPos.x + dstPos.x) / 2, midY = (srcPos.y + dstPos.y) / 2
    return { id: 'F' + String(f.fiberId).padStart(3, '0'), color: f.color, ne: 'NE-' + f.src_ne_id + '→NE-' + f.dst_ne_id, isInter: f.isInter, boards: 'BD-' + f.src_board_id + ' ↔ BD-' + f.dst_board_id, path: `M${srcPos.x + neBoxW/2},${srcPos.y + neBoxH/2} C${midX},${midY} ${midX},${midY} ${dstPos.x + neBoxW/2},${dstPos.y + neBoxH/2}` }
  })
  topologyFibers.value = topoFibers
  topologyAnims.value = topoFibers.filter(f => f.color !== 'g').map((f, i) => ({ fid: f.id, color: f.color === 'r' ? '#ff8a8d' : '#ffd27f', dur: (2 + i * 0.4) + 's' }))
  topologyNEs.value = neList.map(neId => {
    ...
    return { name: 'NE-' + neId, x: pos.x, y: pos.y, active: activeIds.length, passive: passiveIds.length, activeBoards: activeIds.map(id => 'BD-' + id).join(' '), passiveBoards: passiveIds.map(id => 'BD-' + id).join(' ') }
  })
}
...
function newSession() {
  if (currentSession.value) savedSessionMessages.value[currentSession.value] = [...messages.value]
  const newId = 's-' + Date.now().toString().slice(-12)
  sessions.value.unshift({ id: newId, title: '新会话', time: '刚刚 · 进行中' })
  currentSession.value = newId; messages.value = []; saveChatState(); showToast('新建会话成功')
}

watch(currentSession, (newId) => {
  if (!newId) return
  const saved = savedSessionMessages.value[newId]
  messages.value = saved ? [...saved] : []; saveChatState()
})

function quickAction(type) {
  const actions = { colored: '查询所有有颜色连纤', batch: '批量分析所有红色连纤的衰耗', stats: '查询当前连纤统计信息', check: '执行网元巡检' }
  inputText.value = actions[type] || ''
  inputText.value && showToast('已填入指令：' + inputText.value)
}

function analyzeFiber(fid) { inputText.value = '分析光纤 ' + fid + ' 的质量'; showToast('已填入指令 → ' + fid) }
...
function showTopoTip(event, fiber) {
  ...
  topoTip.value = { visible: true, x: Math.min(event.clientX - rect.left + 12, rect.width - 180), y: Math.max(event.clientY - rect.top - 40, 2), text: `${fiber.id} · ${fiber.ne} · ${colorIcon} ${typeLabel}\n${fiber.boards}` }
}

function hideTopoTip() { topoTip.value.visible = false }
...
async function sendMessage() {
  ...
  try {
    await sendChatMessage(userMsg.content, (event) => {
      ...
      else if (event.type === 'tool_call') { aiMsg.tools.push({ tag: event.tool || 'tool', name: event.tool || 'unknown', note: JSON.stringify(event.args || {}).slice(0, 120), ms: 0 }) }
      else if (event.type === 'tool_result') { const content = typeof event.content === 'string' ? event.content : JSON.stringify(event.content); aiMsg.report.push({ c: 'tool', t: `[${event.tool}] ${content.slice(0, 300)}` }) }
      else if (event.type === 'subagent') { aiMsg.tools.push({ tag: 'sub-agent', name: event.agent || 'unknown', note: (event.content || '').slice(0, 120), ms: 0 }) }
      ...
      else if (event.type === 'report') { aiMsg.rawMd += (event.content || '') }
  ...
  } catch (e) { aiMsg.rawMd += '\n❌ 服务暂时不可用，请稍后重试'; aiMsg.done = '❌ 失败'; console.error('Chat error:', e) }
  savedSessionMessages.value[currentSession.value] = [...messages.value]; saveChatState(); sending.value = false
}

function scrollToBottom() { setTimeout(() => { if (chatlog.value) chatlog.value.scrollTop = chatlog.value.scrollHeight }, 50) }
...
function genTrend(range) {
  ...
  const stride = Math.max(1, Math.ceil(N / 120)); const n = Math.floor((N - 1) / stride) + 1
  const now = Date.now(); const seed = { '1h': 0, '6h': 1, '24h': 2, '7d': 3 }[range] || 0
  let wr = 9 + seed * 0.5, wy = 25 + seed * 0.5; const R = [], Y = [], T = []
  for (let i = 0; i < n; i++) {
    const t = new Date(now - (n - 1 - i) * 5 * 60000 * stride)
    const wave = Math.sin((t.getHours() + t.getMinutes() / 60 - 3) / 24 * Math.PI * 2)
    wr += (Math.random() - 0.5) * 1.2; wy += (Math.random() - 0.5) * 1.5
    R.push(Math.max(3, Math.min(19, wr + wave * 3.4))); Y.push(Math.max(14, Math.min(38, wy + wave * 4.6))); T.push(t)
  }
  trendData.value = { red: R, yellow: Y, times: T, anim: 1, hover: null }
}

function drawTrend() {
  const canvas = trendCanvas.value; if (!canvas) return
  const ctx = canvas.getContext('2d'); const w = canvas.clientWidth; const h = canvas.clientHeight
  const dpr = window.devicePixelRatio || 1
  if (canvas.width !== w * dpr) { canvas.width = w * dpr; canvas.height = h * dpr }
  ctx.setTransform(dpr, 0, 0, dpr, 0, 0); ctx.clearRect(0, 0, w, h)
  const L = 30, R = 8, T = 8, B = 20; const n = trendData.value.red.length; if (n === 0) return
  ...
  ctx.font = '9.5px JetBrains Mono'; ctx.fillStyle = '#5b7391'; ctx.strokeStyle = 'rgba(79,141,255,.12)'
  for (let g = 0; g <= 3; g++) { const v = max * g / 3; const y = Y(v); ctx.beginPath(); ctx.moveTo(L, y); ctx.lineTo(w - R, y); ctx.stroke(); ctx.fillText(Math.round(v), 4, y + 3) }
  const step = Math.max(1, Math.floor(n / 5))
  for (let i = 0; i < n; i += step) {
    const lb = trendData.value.times[i]; if (!lb || !(lb instanceof Date) || isNaN(lb.getTime())) continue
    const label = currentRange.value === '7d' ? (lb.getMonth() + 1) + '/' + lb.getDate() : String(lb.getHours()).padStart(2, '0') + ':' + String(lb.getMinutes()).padStart(2, '0')
  ...
  const line = (arr, col, fill) => {
    ...
    ctx.lineTo(X(n - 1), h - B); ctx.lineTo(L, h - B); ctx.closePath()
    const g = ctx.createLinearGradient(0, T, 0, h - B); g.addColorStop(0, fill); g.addColorStop(1, 'rgba(0,0,0,0)'); ctx.fillStyle = g; ctx.fill()
  }
  ...
  if (trendData.value.hover != null) {
    const i = trendData.value.hover; const x = X(i)
    ctx.strokeStyle = 'rgba(220,232,247,.35)'; ctx.setLineDash([3, 3]); ctx.beginPath(); ctx.moveTo(x, T); ctx.lineTo(x, h - B); ctx.stroke(); ctx.setLineDash([])
    ;[[trendData.value.red[i], '#ff5257'], [trendData.value.yellow[i], '#ffb224']].forEach(([v, c]) => { ctx.beginPath(); ctx.arc(x, Y(v), 3.4, 0, 7); ctx.fillStyle = c; ctx.fill(); ctx.strokeStyle = '#0a1424'; ctx.lineWidth = 1.6; ctx.stroke() })
  }
}

function animTrend() {
  trendData.value.anim = 0; const t0 = performance.now()
  const animate = t => { trendData.value.anim = Math.min(1, (t - t0) / 520); trendData.value.anim = 1 - Math.pow(1 - trendData.value.anim, 3); drawTrend(); if (trendData.value.anim < 1) requestAnimationFrame(animate) }
  requestAnimationFrame(animate)
}

function handleTrendMouseMove(event) {
  const canvas = trendCanvas.value; if (!canvas) return
  const rect = canvas.getBoundingClientRect(); const n = trendData.value.red.length; if (n === 0) return
  const i = Math.round((event.clientX - rect.left - 30) / (rect.width - 38) * (n - 1))
  if (isNaN(i) || i < 0 || i >= n) { trendData.value.hover = null; tooltip.value.visible = false; drawTrend(); return }
  trendData.value.hover = i; drawTrend()
  const t = trendData.value.times[i]; if (!t || !(t instanceof Date) || isNaN(t.getTime())) { tooltip.value.visible = false; return }
  const lb = currentRange.value === '7d' ? (t.getMonth() + 1) + '/' + t.getDate() + ' ' + String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0') : String(t.getHours()).padStart(2, '0') + ':' + String(t.getMinutes()).padStart(2, '0')
  tooltip.value = { visible: true, x: Math.min(event.clientX - rect.left + 14, rect.width - 110), y: Math.max(event.clientY - rect.top - 44, 4), time: lb, red: Math.round(trendData.value.red[i]), yellow: Math.round(trendData.value.yellow[i]) }
}

function handleTrendMouseLeave() { trendData.value.hover = null; tooltip.value.visible = false; drawTrend() }
...
watch(currentRange, async (newRange) => {
  ...
  } else { genTrend(newRange) }
  trendData.value.hover = null; tooltip.value.visible = false
  nextTick(() => { drawTrend(); animTrend() })
})

let timeInterval = null

onMounted(async () => {
  updateTime(); timeInterval = setInterval(updateTime, 1000); fetchColoredFibers()
  try { const { data } = await getStatus(); if (data.llm && data.llm.model) llmModel.value = data.llm.model } catch (e) { console.error('Failed to get status:', e) }
  if (currentSession.value && !sessions.value.find(s => s.id === currentSession.value)) {
    sessions.value.unshift({ id: currentSession.value, title: messages.value.length > 0 ? messages.value[0].content?.slice(0, 20) || '会话' : '会话', time: '刚刚' })
  } else if (!currentSession.value) {
    const defaultId = 's-' + Date.now().toString().slice(-12); currentSession.value = defaultId
    sessions.value.unshift({ id: defaultId, title: '新会话', time: '刚刚' })
    savedSessionMessages.value[defaultId] = []; messages.value = []; saveChatState()
  }
  await fetchTrend(24)
  if (trendPoints.value.length > 0) {
    trendData.value.red = trendPoints.value.map(p => p.red_count)
    trendData.value.yellow = trendPoints.value.map(p => p.yellow_count)
    trendData.value.times = trendPoints.value.map(p => new Date(String(p.timestamp).replace(' ', 'T')))
  } else { genTrend(currentRange.value) }
  nextTick(() => { drawTrend(); animTrend() })
})

onBeforeRouteLeave(() => { saveChatState() })

onActivated(() => {
  if (sending.value) {
    sending.value = false
    const lastAi = [...messages.value].reverse().find(m => m.type === 'amsg')
    if (lastAi && !lastAi.done) lastAi.done = '️ 导航中断，请重新发送'
  }
  nextTick(() => { drawTrend() })
})
...
header { height: 54px; flex: none; display: flex; align-items: center; gap: 16px; padding: 0 16px; border-bottom: 1px solid var(--line); background: linear-gradient(180deg, #0d1a2e, #0a1424); position: relative; z-index: 1; }
...
.top-status { margin-left: auto; display: flex; align-items: center; gap: 8px; }
.clock { font-family: var(--mono); font-size: 12px; color: var(--cyan); letter-spacing: .06em; margin-left: 6px; }
.stats-band { flex: none; display: grid; gap: 12px; padding: 12px 16px 4px; grid-template-columns: 308px minmax(0, 1fr) 262px 292px; position: relative; z-index: 1; }
.counters { display: flex; flex-direction: column; padding-bottom: 8px; }
.cnt-row { display: grid; grid-template-columns: 1fr 1fr 1fr; gap: 8px; padding: 0 10px; }
.cnt { border-radius: 6px; padding: 8px 10px 7px; border: 1px solid var(--line); transition: transform .2s; }
.cnt:hover { transform: translateY(-2px); }
.cnt.red { background: linear-gradient(180deg, rgba(255, 82, 87, .16), rgba(255, 82, 87, .04)); border-color: rgba(255, 82, 87, .4); }
.cnt.yel { background: linear-gradient(180deg, rgba(255, 178, 36, .14), rgba(255, 178, 36, .04)); border-color: rgba(255, 178, 36, .4); }
.cnt.tot { background: linear-gradient(180deg, rgba(63, 208, 255, .12), rgba(63, 208, 255, .03)); border-color: rgba(63, 208, 255, .35); }
.cnt label { font-size: 10px; color: var(--tx2); letter-spacing: .1em; }
.cnt b { display: block; font-family: var(--disp); font-weight: 700; font-size: 34px; line-height: 1.05; margin-top: 2px; }
...
.alm-chip { font-size: 10.5px; font-family: var(--mono); padding: 2px 8px; border-radius: 3px; border: 1px solid var(--line); }
...
.ranges button { background: transparent; border: none; color: var(--tx2); font-family: var(--mono); font-size: 11px; padding: 3px 10px; cursor: pointer; transition: .18s; }
...
.ctip { position: absolute; display: block; pointer-events: none; z-index: 5; background: #0a1626; border: 1px solid var(--line2); border-radius: 5px; padding: 6px 9px; font-family: var(--mono); font-size: 11px; box-shadow: 0 6px 16px rgba(0, 0, 0, .5); white-space: nowrap; }
...
.fitem { display: grid; grid-template-columns: 56px 42px 1fr; gap: 6px; align-items: center; font-size: 11px; padding: 4px 6px; border-radius: 4px; }
...
main { flex: 1; min-height: 0; display: grid; gap: 12px; padding: 12px 16px; grid-template-columns: 246px minmax(0, 1fr) 348px; position: relative; z-index: 1; }
.rail { display: flex; flex-direction: column; gap: 12px; min-height: 0; overflow-y: auto; }
.btn-new { flex: none; padding: 9px; border-radius: 6px; cursor: pointer; font-size: 13px; font-weight: 700; border: 1px solid rgba(63, 208, 255, .5); color: var(--cyan); letter-spacing: .1em; background: linear-gradient(180deg, rgba(63, 208, 255, .16), rgba(63, 208, 255, .05)); transition: .2s; }
...
.agrow { display: grid; grid-template-columns: 8px 1fr auto; gap: 8px; align-items: center; padding: 5px 12px; font-size: 11.5px; transition: background .3s; }
.agrow .nm { font-family: var(--mono); color: var(--tx2); }
.agrow .mdl { font-family: var(--mono); font-size: 9.5px; padding: 1px 6px; border-radius: 3px; border: 1px solid var(--line); color: var(--tx3); }
...
.wx-avatar { flex: none; width: 36px; height: 36px; border-radius: 6px; display: grid; place-items: center; font-size: 12px; font-weight: 700; font-family: var(--disp); }
...
.wx-bubble { max-width: 72%; min-width: 80px; padding: 12px 16px; border-radius: 10px; font-size: 13.5px; line-height: 1.7; word-break: break-word; overflow-wrap: break-word; display: flex; flex-direction: column; gap: 8px; }
...
.wx-md :deep(h1), .wx-md :deep(h2), .wx-md :deep(h3) { color: #fff; margin: 10px 0 6px; line-height: 1.4; }
...
.wx-md :deep(pre) { background: #0a1524; border: 1px solid #1c3149; border-radius: 6px; padding: 10px 12px; overflow-x: auto; margin: 8px 0; }
.wx-md :deep(pre code) { background: none; padding: 0; color: #c9d9ef; }
.wx-md :deep(table) { width: 100%; border-collapse: collapse; margin: 8px 0; font-size: 12.5px; }
.wx-md :deep(th), .wx-md :deep(td) { border: 1px solid #1e3554; padding: 6px 10px; text-align: left; }
...
.wx-md :deep(blockquote) { border-left: 3px solid var(--amber); padding: 4px 12px; margin: 8px 0; color: var(--tx2); background: rgba(255, 178, 36, .05); border-radius: 0 4px 4px 0; }
...
.fold > .fhead { display: flex; align-items: center; gap: 8px; padding: 7px 11px; cursor: pointer; user-select: none; font-size: 11.5px; color: var(--tx2); transition: background .15s; }
...
.quick button { background: rgba(79, 141, 255, .08); border: 1px solid var(--line); color: var(--tx2); font-size: 11.5px; padding: 4px 12px; border-radius: 20px; cursor: pointer; transition: .18s; }
.quick button:hover:not(:disabled) { border-color: var(--cyan); color: var(--cyan); transform: translateY(-1px); box-shadow: 0 3px 10px rgba(63, 208, 255, .18); }
...
.topo-toggle button { background: transparent; border: 1px solid var(--line2); color: var(--fg2); font-size: 10px; padding: 2px 8px; cursor: pointer; font-family: var(--mono); transition: all .15s; }
...
.ttip { position: absolute; display: block; pointer-events: none; z-index: 6; background: #0a1626; border: 1px solid var(--line2); border-radius: 5px; padding: 5px 9px; font-family: var(--mono); font-size: 10.5px; box-shadow: 0 6px 16px rgba(0, 0, 0, .5); white-space: pre; }
.fibers-p { flex: 1; min-height: 0; display: flex; flex-direction: column; }
.fchips { display: flex; gap: 6px; padding: 0 12px 8px; }
.fchips button { background: transparent; border: 1px solid var(--line); color: var(--tx2); font-size: 11px; padding: 2px 10px; border-radius: 3px; cursor: pointer; transition: .15s; font-family: var(--mono); }
...
.frow { display: grid; grid-template-columns: 12px 46px 1fr 54px 40px; gap: 7px; align-items: center; padding: 6px 8px; border-radius: 5px; cursor: pointer; border-left: 2px solid transparent; transition: .15s; font-size: 11.5px; }
...
footer { flex: none; padding: 7px 16px; border-top: 1px solid var(--line); font-size: 10.5px; color: var(--tx3); font-family: var(--mono); display: flex; gap: 18px; background: #0a1424; flex-wrap: wrap; position: relative; z-index: 1; }
footer b { color: var(--tx2); font-weight: 500; }
.toast { position: fixed; left: 50%; bottom: 44px; transform: translateX(-50%) translateY(20px); z-index: 99; background: #0d1e34; border: 1px solid var(--line2); color: var(--tx); padding: 8px 18px; border-radius: 6px; font-size: 12px; opacity: 0; pointer-events: none; transition: .3s; box-shadow: 0 8px 24px rgba(0, 0, 0, .5); }
.toast.show { opacity: 1; transform: translateX(-50%); }
.obs-link { cursor: pointer; color: #3fd0ff; border-color: rgba(63,208,255,.4); background: rgba(63,208,255,.08); text-decoration: none; transition: .2s; display: flex; align-items: center; gap: 4px; }
.obs-link:hover { background: rgba(63,208,255,.18); box-shadow: 0 0 12px rgba(63,208,255,.25); }
.rag-link { cursor: pointer; color: #2fd6a3; border-color: rgba(47,214,163,.4); background: rgba(47,214,163,.08); text-decoration: none; transition: .2s; display: flex; align-items: center; gap: 4px; }
.rag-link:hover { background: rgba(47,214,163,.18); box-shadow: 0 0 12px rgba(47,214,163,.25); }
@media (max-width: 1400px) { main { grid-template-columns: 220px minmax(0, 1fr) 300px; } }
@media (max-width: 1200px) { main { grid-template-columns: 200px minmax(0, 1fr) 280px; } .stats-band { grid-template-columns: 260px minmax(0, 1fr) 220px 240px; } }
@media (max-width: 1024px) { main { grid-template-columns: 1fr; grid-template-rows: auto auto auto; } .rail { order: 2; } .chat { order: 1; min-height: 400px; } .fibers-p { order: 3; } .stats-band { grid-template-columns: 1fr 1fr; } .top-status { flex-wrap: wrap; gap: 4px; } .top-status .pill { font-size: 10px; padding: 2px 6px; } }
@media (max-width: 768px) { header { height: auto; min-height: 54px; flex-wrap: wrap; padding: 8px 12px; } .brand h1 { font-size: 14px; } .stats-band { grid-template-columns: 1fr; } .cnt-row { grid-template-columns: 1fr; } .chat-head .mw { display: none; } }
</style>
